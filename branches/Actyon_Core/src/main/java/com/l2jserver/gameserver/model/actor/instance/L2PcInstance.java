...
public final class L2PcInstance extends L2Playable
{
	...
	private static final int NEVIT_HOURGLASS_PAUSED = 10;
	private static final int NEVIT_HOURGLASS_MAINTAIN = 1;
	...
	private int _recomHave; // The number of recommendation obtained by the player.
	private int _recomLeft;// The number of recommendation that the player can give.
	private ScheduledFuture<?> _nevitHourglassTask;// Recommendation Bonus task
	private ScheduledFuture<?> _recoGiveTask; // Recommendation task
	private boolean _recoTwoHoursGiven = false; // Recommendation Two Hours bonus
	private int _recomBonusTime = 3600; // Recommendation Time Init.
	...
	public static L2PcInstance load(int objectId)
	{
		try
		{
			...
			// Init Task give = 10 reco 2hs & 1 every 1hs.
			player.startRecoGiveTask();
			
			// Lo cargaré en L2GameClient
			// Check Restore reco at 06:30:00 hs.
			player.checkRecom();
	...
	public void revalidateZone(boolean force)
	{
		...
		// zeus-------------------------------------
		else if (isInsideZone(ZoneId.PEACE) && !allowPeaceAttack())
		{
	...
	public void standUp()
	{
		...
		// zeus---------------------------------------------------------------------------------------------
		else if (ZeuS.isSellingBuf(this))
		{
			sendMessage("A dark force beyond your mortal understanding makes your knees to shake when you try to stand up...");
			return;
		}
	...
	public void doAttack(L2Character target)
	{
		...
		{
			if (ZeuS.isLevelProtected(this, (L2PcInstance) target))
			{
	...
	public void tryOpenPrivateBuyStore()
	{
		...
		if (canOpenPrivateStore())
		{
			...
			if (getPrivateStoreType() == PrivateStoreType.NONE)
			{
				...
				ZeuS.setMerchant(this, false); // zeus
			}
		}
		else
		{
	...
	public void tryOpenPrivateSellStore(boolean isPackageSale)
	{
		...
		if (canOpenPrivateStore())
		{
			...
			if (getPrivateStoreType() == PrivateStoreType.NONE)
			{
				...
				ZeuS.setMerchant(this, true); // zeus
			}
		}
		else
		{
		...
		if (killer != null)
		{
			...
			else
			{
				...
				if ((pk == null) || !pk.isCursedWeaponEquipped())
				{
					...
					// zeus
					if (!isLucky() && (insideSiegeZone || !insidePvpZone) && !isInTownWarEvent())
					{
	...
	public void increasePvpKills(L2Character target)
	{
		...
		if ((target instanceof L2PcInstance) && AntiFeedManager.getInstance().check(this, target))
		{
			...
			// zeus
			ZeuS.logPvP_PK(target.getActingPlayer(), "PVP", this);
			
			// Faction System
			if (Config.FACTION_SYSTEM_ENABLED && Config.FACTION_ALLOW_PVP_REWARD)
			{
				if (getPvpKills() > 0)
				{
					this.sendMessage(".: Receive Faction PvP Reward :.");
					addItem("Reward for PvP kill:", Config.FACTION_GIVE_ITEM_FOR_PVP_KILL, Config.FACTION_GIVE_ITEM__COUNT_FOR_PVP_KILL, this, true);
				}
			}
			if (Config.FACTION_SYSTEM_ENABLED && Config.FACTION_PVP_SYSTEM_QUAKE)
			{
				quakeSystem++;
				switch (quakeSystem)
				{
					case 2:
						this.sendPacket(Music.DOUBLE_KILL.getPacket());
						Broadcast.toAllOnlinePlayersOnScreen("" + this.getName() + " has scored a Double Kill !! ", 5000);
						Broadcast.toAllOnlinePlayers("" + this.getName() + " has scored a Double Kill !! ", true);
						break;
					
					case 3:
						this.sendPacket(Music.TRIPLE_KILL.getPacket());
						Broadcast.toAllOnlinePlayersOnScreen("" + this.getName() + " has scored a Triple Kill !! ", 5000);
						Broadcast.toAllOnlinePlayers("" + this.getName() + " has scored a Triple Kill !! ", true);
						break;
					
					case 4:
						this.sendPacket(Music.QUADRA_KILL.getPacket());
						Broadcast.toAllOnlinePlayersOnScreen("" + this.getName() + " has scored a Quadra Kill !! ", 5000);
						Broadcast.toAllOnlinePlayers("" + this.getName() + " has scored a Double Kill !! ", true);
						break;
					
					case 5:
						this.sendPacket(Music.PENTA_KILL.getPacket());
						Broadcast.toAllOnlinePlayersOnScreen("" + this.getName() + " has scored a Penta Kill !! ", 5000);
						Broadcast.toAllOnlinePlayers("" + this.getName() + " has scored a Double Kill !! ", true);
						break;
					
					case 6:
						this.sendPacket(Music.LENGENDARY_KILL.getPacket());
						Broadcast.toAllOnlinePlayersOnScreen("" + this.getName() + " has scored a Legendary Kill !! ", 5000);
						Broadcast.toAllOnlinePlayers("" + this.getName() + " has scored a Double Kill !! ", true);
						break;
				}
			}
	...
	public void increasePkKillsAndKarma(L2Character target)
	{
		...
		if (target.isPlayer())
		{
			...
			// zeus
			ZeuS.logPvP_PK(target.getActingPlayer(), "PK", this);
		}
	...
	public boolean useMagic(Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		// zeus--------------------------------------------------------------------------------
		if ((target != this) && (target instanceof L2PcInstance) && skill.isBad() && (ZeuS.getInstance().isLevelProtected(this, (L2PcInstance) target)))
		{
			setIsCastingNow(false);
			sendPacket(ActionFailed.STATIC_PACKET);
			return false;
	...
	private boolean checkUseMagicConditions(Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (skill.isBad())
		{
			...
			// zeus
			if (!target.canBeAttacked() && allowPeaceAttack() && !target.isDoor())
			{
	...
	public void reviveRequest(L2PcInstance reviver, Skill skill, boolean Pet, int power)
	{
		...
		if ((Pet && hasPet() && getSummon().isDead()) || (!Pet && isDead()))
		{
			...
			if (isInTownWarEvent())
			{
				...
				SkillData.getInstance().getSkill(1323, 1).applyEffects(this, this);
			}
	...
	public void storeRecommendations(boolean cancelRecoTask)
	{
			...
			PreparedStatement statement = con.prepareStatement("INSERT INTO character_reco_bonus (charId,rec_have,rec_left,time_left) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE rec_have=?, rec_left=?, time_left=?"))
		{
			...
			statement.setLong(7, getNevitHourglassTime());// getStat().hasPausedNevitHourglass() ? loadReco() : getNevitHourglassTime());
			statement.execute();
			statement.close();
		}
		catch (Exception e)
		{
			LOG.warn("Could not update Recommendations for player: " + getObjectId(), e);
		}
	}
	...
	private void loadRecommendations()
	{
		...
		{
			LOG.warn("Could not restore Recommendations for player: " + getObjectId(), e);
		}
	}
	
	@SuppressWarnings("unused")
	private long loadReco()
	{
		long timeLeft = 0;
		try (Connection con = ConnectionFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT rec_have,rec_left,time_left FROM character_reco_bonus WHERE charId=? LIMIT 1"))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				if (rset.next())
				{
					setRecomHave(rset.getInt("rec_have"));
					setRecomLeft(rset.getInt("rec_left"));
					timeLeft = rset.getLong("time_left");
				}
			}
		}
		catch (Exception e)
	...
	/*
	 * Anulo esta comprobación y utilizo el task del NevitSystem.java . reutilizando el reset de las 06:30:00 hs.
	 */
	public void checkRecom()
	{
		...
		long count = Math.round(((System.currentTimeMillis() / 1000) - getLastAccess()) / 86400);
		if ((count == 0) && (getLastAccess() < (temp.getTimeInMillis() / 1000)) && (System.currentTimeMillis() > temp.getTimeInMillis()))
		{
			count++;
		}
	...
	public void restartRecom()
	{
		setRecomHave(getRecomHave() - 20);
		setRecomLeft(20);
		setNevitHourglassTime(3600);
		stopNevitHourglassTask(false);
	...
	{
		if ((_nevitHourglassTask == null) && (getNevitHourglassTime() > 0) && (getNevitHourglassStatus() == NEVIT_HOURGLASS_ACTIVE))
		{
			_nevitHourglassTask = ThreadPoolManager.getInstance().scheduleGeneral(new RecoBonusTaskEnd(this), getNevitHourglassTime() * 1000);
		}
	}
	
	public void stopNevitHourglassTask(boolean saveTime)
	{
		if (_nevitHourglassTask != null)
		{
			if (saveTime)
	...
	public void setNevitHourglassTime(int val)
	{
		_recomBonusTime = val;
	}
