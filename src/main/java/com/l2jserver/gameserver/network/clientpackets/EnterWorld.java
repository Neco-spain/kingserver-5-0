	...
	protected void runImpl()
	{
		...
		if (!Config.FACTION_SYSTEM_ENABLED)
		{
			if (activeChar.isGood())
			{
				...
				activeChar.setTitle("");
				activeChar.sendMessage("Welcome " + activeChar.getName() + ". Faction times finish...");
				activeChar.sendPacket(new ExShowScreenMessage("Welcome " + activeChar.getName() + ". Faction times finish...", 10000));
				activeChar.resetEvilGood();
		...
		if (showClanNotice)
		{
			...
			// sendPacket(notice);
			// zeus-------------------------------------------------------------------------
			if (!ZeuS.ShowAnuncio_PIN(true, activeChar))
			{
				NpcHtmlMessage notice = new NpcHtmlMessage(1);
				notice.setFile(activeChar.getHtmlPrefix(), "data/html/clanNotice.htm");
				notice.replace("%clan_name%", activeChar.getClan().getName());
				notice.replace("%notice_text%", activeChar.getClan().getNotice());
				notice.replace("%name%", activeChar.getName());
				notice.disableValidation();
				sendPacket(notice);
			}
		}
		...
		if (Config.SERVER_NEWS && !showClanNotice)
		{
			...
			// if (serverNews != null)
			if (!ZeuS.ShowAnuncio_PIN(false, activeChar))
			{
				// sendPacket(new NpcHtmlMessage(serverNews));
				NpcHtmlMessage serverNews = new NpcHtmlMessage(1);
				serverNews.setFile(activeChar.getHtmlPrefix(), "data/html/servnews.htm");
				serverNews.disableValidation();
				if (serverNews != null)
				{
					sendPacket(serverNews);
				}
			}
		}
		...
		// zeus ---------------------------------------------------------------------------------------------------
		if (activeChar.isInsideZone(ZoneId.NO_RESTART) && !activeChar.isGM())
		{
